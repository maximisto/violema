...
- **WP-0.1 first hardening pass shipped on this PR branch:** authenticated users already get server-owned `workspaceIds`/`defaultWorkspaceId`, `resolveWorkspaceContext` treats client workspace values as selectors and 403s cross-tenant access, automation routes filter by workspace, and Slack event callbacks now resolve team/channel to a stored user Slack connection instead of falling back to the default workspace. Express-level tenant acceptance coverage now verifies cross-tenant 403s for settings, platform tasks, automations, and billing. Remaining scope is explicit `owner_user_id` on automations plus secondary route coverage for task runs, ledgers, Studio, and workflow ledgers.
